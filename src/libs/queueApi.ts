...
const api = axios.create({
  ...
  params: {
    tenxa: 'xadongtho' // Default parameter for all requests
  }
});
...
export const getWaitingTickets = async (): Promise<WaitingTicketsResponse> => {
  try {
    ...
    const possiblePaths = [
      ...
      '/tickets/waiting?tenxa=xadongtho',        // Current path
      '/tickets?tenxa=xadongtho',               // Base tickets endpoint
...
export const createTicket = async (request: CreateTicketRequest): Promise<CreateTicketResponse> => {
  try {
    ...
    const response = await api.post<CreateTicketResponse>('/tickets?tenxa=xadongtho', request);
