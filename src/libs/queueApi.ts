...
const api = axios.create({
  ...
  params: {
    tenxa: 'xatruongsinh' // Default parameter for all requests
  }
});
...
export const getWaitingTickets = async (): Promise<WaitingTicketsResponse> => {
  try {
    ...
    const possiblePaths = [
      ...
      '/tickets/waiting?tenxa=xatruongsinh',        // Current path
      '/tickets?tenxa=xatruongsinh',               // Base tickets endpoint
...
export const createTicket = async (request: CreateTicketRequest): Promise<CreateTicketResponse> => {
  try {
    ...
    const response = await api.post<CreateTicketResponse>('/tickets?tenxa=xatruongsinh', request);
