...
rootApi.interceptors.request.use(
  (config) => {
    ...
    if (token) {
      ...
      if (config.url?.includes('/call-next?tenxa=xavinhtuy')) {
...
export const authsAPI = {
  ...
  createUser: (userData: CreateUserRequest): Promise<User> => {
    return rootApi.post('/auths/users', userData, { params: { tenxa: 'xavinhtuy' } }).then(response => response.data);
  },
  ...
  getCurrentUser: (): Promise<User> => {
    return rootApi.get('/auths/me', { params: { tenxa: 'xavinhtuy' } }).then(response => response.data);
  },
};
...
export const ttsAPI = {
  ...
  generateAudio: async (request: TTSRequest): Promise<Blob> => {
    const response = await fetch(`${BASE_URL}/tts/`, {
      ...
      body: JSON.stringify({ ...request, tenxa: 'xavinhtuy' })
    });
  ...
  generateCounterAudio: async (request: { counter_id: number; name: string }): Promise<string> => {
    const response = await fetch(`${BASE_URL}/tts/generate_counter_audio?tenxa=xavinhtuy`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export const proceduresAPI = {
  ...
  getProcedures: (search?: string): Promise<Procedure[]> => {
    ...
    return rootApi.get('/procedures', { params: { ...params, tenxa: 'xavinhtuy' } }).then(response => response.data);
  },
  ...
  searchExtended: (search?: string): Promise<ProcedureExtended[]> => {
    const params = search ? { search } : {};
    return rootApi.get('/procedures/search-extended', { params: { ...params, tenxa: 'xavinhtuy' } }).then(response => response.data);
  },
};
...
export const ticketsAPI = {
  ...
  createTicket: (request: CreateTicketRequest): Promise<Ticket> => {
    return rootApi.post('/tickets', request, { params: { tenxa: 'xavinhtuy' } }).then(response => response.data);
  },
  ...
  getWaitingTickets: (): Promise<WaitingTicketsResponse> => {
    return rootApi.get('/tickets/waiting', { params: { tenxa: 'xavinhtuy' } }).then(response => response.data);
  },
...
export const seatsAPI = {
  ...
  getSeats: (): Promise<Seat[]> => {
    return rootApi.get('/seats', { params: { tenxa: 'xavinhtuy' } }).then(response => response.data);
  },
...
export const countersAPI = {
  ...
  getCounters: (): Promise<Counter[]> => {
    return rootApi.get('/counters', { params: { tenxa: 'xavinhtuy' } }).then(response => response.data);
  },
  ...
  callNext: (counterId: number): Promise<CallNextResponse> => {
    return rootApi.post(`/counters/${counterId}/call-next`, null, { params: { tenxa: 'xavinhtuy' } }).then(response => response.data);
  },
  ...
  pauseCounter: (counterId: number, request: PauseCounterRequest): Promise<PauseCounterResponse> => {
    return rootApi.post(`/counters/${counterId}/pause`, request, { params: { tenxa: 'xavinhtuy' } }).then(response => response.data);
  },
  ...
  resumeCounter: (counterId: number): Promise<ResumeCounterResponse> => {
    return rootApi.put(`/counters/${counterId}/resume`, null, { params: { tenxa: 'xavinhtuy' } }).then(response => response.data);
  },
  ...
  upsertCounter: (data: UpsertCounterRequest): Promise<{ id: number; name: string; status: string}> => {
    return rootApi.post('/counters/upsert-counter', data, { params: { tenxa: 'xavinhtuy' } }).then(res => res.data);
  },
  ...
  deleteCounter: (counter_id: number): Promise<string> => {
    return rootApi.delete('/counters/delete-counter', { params: { tenxa: 'xavinhtuy', counter_id } }).then(res => res.data);
  },
};
...
export const tvGroupsAPI = {
  ...
  getGroups: async (): Promise<TVGroup[]> => {
    ...
    try {
      const res = await rootApi.get('/tv-groups', { params: { tenxa: 'xavinhtuy' } });
      return res.data;
    } catch (err) {
  ...
  getGroup: async (id: number): Promise<TVGroup> => {
    try {
      const res = await rootApi.get(`/tv-groups/${id}`, { params: { tenxa: 'xavinhtuy' } });
      return res.data;
    } catch (err) {
  ...
  createGroup: async (data: { name: string; counter_ids: number[]; description?: string }): Promise<TVGroup> => {
    try {
      const res = await rootApi.post('/tv-groups', data, { params: { tenxa: 'xavinhtuy' } });
      // broadcast update
  ...
  updateGroup: async (id: number, data: { name: string; counter_ids: number[]; description?: string }): Promise<TVGroup> => {
    try {
      const res = await rootApi.put(`/tv-groups/${id}`, data, { params: { tenxa: 'xavinhtuy' } });
  ...
  deleteGroup: async (id: number): Promise<string> => {
    try {
      const res = await rootApi.delete(`/tv-groups/${id}`, { params: { tenxa: 'xavinhtuy' } });
...
export const statsDashboardAPI = {
  ...
  getTicketsPerCounter: (params?: { start_date?: string; end_date?: string }) =>
    rootApi.get('/stats/tickets-per-counter', { params: { ...params, tenxa: 'xavinhtuy' } }).then(res => res.data),
  ...
  getAttendedTickets: (params?: { start_date?: string; end_date?: string }) =>
    rootApi.get('/stats/attended-tickets', { params: { ...params, tenxa: 'xavinhtuy' } }).then(res => res.data),
  ...
  getAverageHandlingTime: (params?: { start_date?: string; end_date?: string }) =>
    rootApi.get('/stats/average-handling-time', { params: { ...params, tenxa: 'xavinhtuy' } }).then(res => res.data),
  ...
  getAverageWaitingTime: (params?: { start_date?: string; end_date?: string }) =>
    rootApi.get('/stats/average-waiting-time', { params: { ...params, tenxa: 'xavinhtuy' } }).then(res => res.data),
  ...
  getAfkDuration: (params?: { start_date?: string; end_date?: string }) =>
    rootApi.get('/stats/afk-duration', { params: { ...params, tenxa: 'xavinhtuy' } }).then(res => res.data),
  ...
  getWorkingTimeCheck: (params?: { date_check?: string }) =>
    rootApi.get('/stats/working-time-check', { params: { ...params, tenxa: 'xavinhtuy' } }).then(res => res.data),
  /**
  ...
  getRatingPerCounter: (params?: { start_date?: string; end_date?: string }) =>
    rootApi.get('/stats/rating-per-counter', { params: { ...params, tenxa: 'xavinhtuy' } }).then(res => res.data as RatingPerCounter[]),
  ...
  getFeedbacks: (params?: { rating?: string; counter_id?: number; start_date?: string; end_date?: string }) =>
    rootApi.get('/stats/feedbacks', { params: { ...params, tenxa: 'xavinhtuy' } }).then(res => res.data as FeedbackItem[]),
};
