...
export const counterAPI = {
  // Get all counters
  getCounters: () => axiosInstance.get('/counters', { params: { tenxa: 'xahoaan' } }),
  ...
  getStats: (counterId: string, period?: string) =>
    axiosInstance.get(`/counters/${counterId}/stats`, { params: { period, tenxa: 'xahoaan' } }),
};

// User Management API
export const userAPI = {
  // Get all users
  getUsers: () => axiosInstance.get('/users', { params: { tenxa: 'xahoaan' } }),

  // Create new user
  createUser: (userData: any) => axiosInstance.post('/users', userData, { params: { tenxa: 'xahoaan' } }),

  // Update user
  updateUser: (userId: string, userData: any) =>
    axiosInstance.patch(`/users/${userId}`, userData, { params: { tenxa: 'xahoaan' } }),

  // Delete user
  deleteUser: (userId: string) => axiosInstance.delete(`/users/${userId}`, { params: { tenxa: 'xahoaan' } }),
  ...
  login: (credentials: { username: string; password: string }) => {
    ...
    return axiosInstance.post('/auths/login?tenxa=xahoaan', formData, {
      headers: {
  ...
  register: (userData: { username: string; password: string }) =>
    axiosInstance.post('/auths/users', userData, { params: { tenxa: 'xahoaan' } }),

  // Get current user info
  getCurrentUser: () =>
    axiosInstance.get('/auths/me', { params: { tenxa: 'xahoaan' } }),

  // Logout
  logout: () => axiosInstance.post('/auth/logout', null, { params: { tenxa: 'xahoaan' } }),
};
...
export const proceduresAPI = {
  ...
  getProcedures: (search?: string): Promise<{ data: Procedure[] }> => {
    const params = search ? { search, tenxa: 'xahoaan' } : { tenxa: 'xahoaan' };
...
export const ticketsAPI = {
  ...
  createTicket: (data: CreateTicketRequest): Promise<{ data: Ticket }> => {
    return axiosInstance.post('/tickets/', data, { params: { tenxa: 'xahoaan' } });
  },
};
...
export const countersAPI = {
  ...
  callNext: (counterId: number): Promise<{ data: CallNextResponse }> => {
    return axiosInstance.post(`/counters/${counterId}/call-next`, null, { params: { tenxa: 'xahoaan' } });
  },
  ...
  pauseCounter: (counterId: number, reason: string): Promise<{ data: PauseCounterResponse }> => {
    return axiosInstance.post(`/counters/${counterId}/pause`, { reason }, { params: { tenxa: 'xahoaan' } });
  },
  
  // ▶️ [PUT] `/counters/{counter_id}/resume` – Tiếp tục quầy
  resumeCounter: (counterId: number): Promise<{ data: ResumeCounterResponse }> => {
    return axiosInstance.put(`/counters/${counterId}/resume`, null, { params: { tenxa: 'xahoaan' } });
  },
};
