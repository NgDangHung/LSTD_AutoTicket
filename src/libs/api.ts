...
export const counterAPI = {
  // Get all counters
  getCounters: () => axiosInstance.get('/counters', { params: { tenxa: 'phuonghagiang2' } }),
  ...
  getStats: (counterId: string, period?: string) =>
    axiosInstance.get(`/counters/${counterId}/stats`, { params: { period, tenxa: 'phuonghagiang2' } }),
};

// User Management API
export const userAPI = {
  // Get all users
  getUsers: () => axiosInstance.get('/users', { params: { tenxa: 'phuonghagiang2' } }),

  // Create new user
  createUser: (userData: any) => axiosInstance.post('/users', userData, { params: { tenxa: 'phuonghagiang2' } }),

  // Update user
  updateUser: (userId: string, userData: any) =>
    axiosInstance.patch(`/users/${userId}`, userData, { params: { tenxa: 'phuonghagiang2' } }),

  // Delete user
  deleteUser: (userId: string) => axiosInstance.delete(`/users/${userId}`, { params: { tenxa: 'phuonghagiang2' } }),
  ...
  login: (credentials: { username: string; password: string }) => {
    ...
    return axiosInstance.post('/auths/login?tenxa=phuonghagiang2', formData, {
      headers: {
  ...
  register: (userData: { username: string; password: string }) =>
    axiosInstance.post('/auths/users', userData, { params: { tenxa: 'phuonghagiang2' } }),

  // Get current user info
  getCurrentUser: () =>
    axiosInstance.get('/auths/me', { params: { tenxa: 'phuonghagiang2' } }),

  // Logout
  logout: () => axiosInstance.post('/auth/logout', null, { params: { tenxa: 'phuonghagiang2' } }),
};
...
export const proceduresAPI = {
  ...
  getProcedures: (search?: string): Promise<{ data: Procedure[] }> => {
    const params = search ? { search, tenxa: 'phuonghagiang2' } : { tenxa: 'phuonghagiang2' };
...
export const ticketsAPI = {
  ...
  createTicket: (data: CreateTicketRequest): Promise<{ data: Ticket }> => {
    return axiosInstance.post('/tickets/', data, { params: { tenxa: 'phuonghagiang2' } });
  },
};
...
export const countersAPI = {
  ...
  callNext: (counterId: number): Promise<{ data: CallNextResponse }> => {
    return axiosInstance.post(`/counters/${counterId}/call-next`, null, { params: { tenxa: 'phuonghagiang2' } });
  },
  ...
  pauseCounter: (counterId: number, reason: string): Promise<{ data: PauseCounterResponse }> => {
    return axiosInstance.post(`/counters/${counterId}/pause`, { reason }, { params: { tenxa: 'phuonghagiang2' } });
  },
  
  // ▶️ [PUT] `/counters/{counter_id}/resume` – Tiếp tục quầy
  resumeCounter: (counterId: number): Promise<{ data: ResumeCounterResponse }> => {
    return axiosInstance.put(`/counters/${counterId}/resume`, null, { params: { tenxa: 'phuonghagiang2' } });
  },
};
