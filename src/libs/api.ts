...
export const counterAPI = {
  // Get all counters
  getCounters: () => axiosInstance.get('/counters', { params: { tenxa: 'xabinhxa' } }),
  ...
  getStats: (counterId: string, period?: string) =>
    axiosInstance.get(`/counters/${counterId}/stats`, { params: { period, tenxa: 'xabinhxa' } }),
};

// User Management API
export const userAPI = {
  // Get all users
  getUsers: () => axiosInstance.get('/users', { params: { tenxa: 'xabinhxa' } }),

  // Create new user
  createUser: (userData: any) => axiosInstance.post('/users', userData, { params: { tenxa: 'xabinhxa' } }),

  // Update user
  updateUser: (userId: string, userData: any) =>
    axiosInstance.patch(`/users/${userId}`, userData, { params: { tenxa: 'xabinhxa' } }),

  // Delete user
  deleteUser: (userId: string) => axiosInstance.delete(`/users/${userId}`, { params: { tenxa: 'xabinhxa' } }),
  ...
  login: (credentials: { username: string; password: string }) => {
    ...
    return axiosInstance.post('/auths/login?tenxa=xabinhxa', formData, {
      headers: {
  ...
  register: (userData: { username: string; password: string }) =>
    axiosInstance.post('/auths/users', userData, { params: { tenxa: 'xabinhxa' } }),

  // Get current user info
  getCurrentUser: () =>
    axiosInstance.get('/auths/me', { params: { tenxa: 'xabinhxa' } }),

  // Logout
  logout: () => axiosInstance.post('/auth/logout', null, { params: { tenxa: 'xabinhxa' } }),
};
...
export const proceduresAPI = {
  ...
  getProcedures: (search?: string): Promise<{ data: Procedure[] }> => {
    const params = search ? { search, tenxa: 'xabinhxa' } : { tenxa: 'xabinhxa' };
...
export const ticketsAPI = {
  ...
  createTicket: (data: CreateTicketRequest): Promise<{ data: Ticket }> => {
    return axiosInstance.post('/tickets/', data, { params: { tenxa: 'xabinhxa' } });
  },
};
...
export const countersAPI = {
  ...
  callNext: (counterId: number): Promise<{ data: CallNextResponse }> => {
    return axiosInstance.post(`/counters/${counterId}/call-next`, null, { params: { tenxa: 'xabinhxa' } });
  },
  ...
  pauseCounter: (counterId: number, reason: string): Promise<{ data: PauseCounterResponse }> => {
    return axiosInstance.post(`/counters/${counterId}/pause`, { reason }, { params: { tenxa: 'xabinhxa' } });
  },
  
  // ▶️ [PUT] `/counters/{counter_id}/resume` – Tiếp tục quầy
  resumeCounter: (counterId: number): Promise<{ data: ResumeCounterResponse }> => {
    return axiosInstance.put(`/counters/${counterId}/resume`, null, { params: { tenxa: 'xabinhxa' } });
  },
};
