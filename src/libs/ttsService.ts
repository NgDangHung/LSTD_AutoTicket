...
export class TTSService {
  ...
  private constructor() {
    ...
    if (typeof window !== 'undefined') {
      ...
      this.loadCounters('xahoangsuphi');
  ...
  async downloadAudio(counterId: number, ticketNumber: number): Promise<void> {
    try {
      const audioBlob = await ttsAPI.generateAudio(counterId, ticketNumber, 'xahoangsuphi');
