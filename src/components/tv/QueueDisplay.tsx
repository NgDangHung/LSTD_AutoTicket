...
export default function QueueDisplay() {
  ...
  const fetchServingTicket = async (counterId: number): Promise<RealTicket | null> => {
    try {
      const response = await rootApi.get(`/tickets/called`, { params: { counter_id: counterId, tenxa: 'xaminhthanh' } });
  ...
  const fetchConfig = useCallback(async () => {
    try {
      const data = await configAPI.getConfig('xaminhthanh');
  ...
  async function handleSaveConfig(newConfig: { header: string; work_time: string; hotline: string }, onSuccess?: () => void, onError?: (err: any) => void) {
    try {
      ...
      await configAPI.setConfig('xaminhthanh', {
  ...
  const fetchCounters = useCallback(async () => {
      try {
        const response = await rootApi.get('/counters/', { params: { tenxa: 'xaminhthanh' } });
  ...
  const fetchAllTickets = async (): Promise<RealTicket[]> => {
    try {
      ...
      const response = await rootApi.get('/tickets/waiting', { params: { tenxa: 'xaminhthanh' } });
      const tickets: RealTicket[] = response.data;
      ...
      for (let counterId = 1; counterId <= 4; counterId++) {
        try {
          const response = await rootApi.get('/tickets/waiting', { params: { counter_id: counterId, tenxa: 'xaminhthanh' } });
      ...
      try {
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (eventData.tenxa !== 'xaminhthanh') return
