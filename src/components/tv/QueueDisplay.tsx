...
export default function QueueDisplay() {
  ...
  const fetchServingTicket = async (counterId: number): Promise<RealTicket | null> => {
    try {
      const response = await rootApi.get(`/tickets/called`, { params: { counter_id: counterId, tenxa: 'xahamyen' } });
  ...
  const fetchConfig = useCallback(async () => {
    try {
      const data = await configAPI.getConfig('xahamyen');
  ...
  async function handleSaveConfig(newConfig: { header: string; work_time: string; hotline: string }, onSuccess?: () => void, onError?: (err: any) => void) {
    try {
      ...
      await configAPI.setConfig('xahamyen', {
  ...
  const fetchCounters = useCallback(async () => {
      try {
        const response = await rootApi.get('/counters/', { params: { tenxa: 'xahamyen' } });
  ...
  const fetchAllTickets = async (): Promise<RealTicket[]> => {
    try {
      ...
      const response = await rootApi.get('/tickets/waiting', { params: { tenxa: 'xahamyen' } });
      const tickets: RealTicket[] = response.data;
      ...
      for (let counterId = 1; counterId <= 4; counterId++) {
        try {
          const response = await rootApi.get('/tickets/waiting', { params: { counter_id: counterId, tenxa: 'xahamyen' } });
      ...
      try {
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (eventData.tenxa !== 'xahamyen') return
      ...
      <>
        <div className="flex justify-between items-center" style={{flexDirection: 'row-reverse'}}>
          <h2 className="text-2xl text-red-700 font-bold italic" style={{position: 'relative',top: '-50px',left: '-180px', fontSize: '2rem'}}>
             ...
             Xã Hàm Yên,  Ngày {new Date().toLocaleDateString('vi-VN')}
          </h2>
        </div>
      </>
