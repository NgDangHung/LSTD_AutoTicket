...
export default function QueueDisplay() {
  ...
  const fetchServingTicket = async (counterId: number): Promise<RealTicket | null> => {
    try {
      const response = await rootApi.get(`/tickets/called`, { params: { counter_id: counterId, tenxa: 'xatanan' } });
  ...
  const fetchConfig = useCallback(async () => {
    try {
      const data = await configAPI.getConfig('xatanan');
  ...
  async function handleSaveConfig(newConfig: { header: string; work_time: string; hotline: string }, onSuccess?: () => void, onError?: (err: any) => void) {
    try {
      ...
      await configAPI.setConfig('xatanan', {
  ...
  const fetchCounters = useCallback(async () => {
      try {
        const response = await rootApi.get('/counters/', { params: { tenxa: 'xatanan' } });
  ...
  const fetchAllTickets = async (): Promise<RealTicket[]> => {
    try {
      ...
      const response = await rootApi.get('/tickets/waiting', { params: { tenxa: 'xatanan' } });
      const tickets: RealTicket[] = response.data;
      ...
      for (let counterId = 1; counterId <= 4; counterId++) {
        try {
          const response = await rootApi.get('/tickets/waiting', { params: { counter_id: counterId, tenxa: 'xatanan' } });
      ...
      try {
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (eventData.tenxa !== 'xatanan') return
